...
from src.exp.exp019 import config as my_config
...
def train_one_epoch(
    ...
    for i, batch in pbar:
        ...
        with amp.autocast_mode.autocast(enabled=use_amp):
            ...
            # 混ぜるのはモデルの内部でspectrogramを作成した後にする
            # if mixupper.do_mixup:
            #     x = mixupper.lam * x + (1.0 - mixupper.lam) * x.flip(0)

            bboxes, k_list, a_list = my_preprocessings.prepare_cutmix(
                x_max=10000 // model.params.stride, y_max=model.params.base_filters, bs=x.size(0)
            )
            out = model(
                x,
        ...
        if mixupper.do_mixup:
            loss *= mixupper.lam
            loss += (1.0 - mixupper.lam) * loss_fn(F.log_softmax(logits, dim=1), out["y"].to(dtype=logits.dtype))
        #     loss *= mixupper.lam
