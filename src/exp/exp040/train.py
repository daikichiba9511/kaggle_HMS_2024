...
import wandb
from src.exp.exp040 import config as my_config
    ...
    for i, batch in pbar:
        ...
        with amp.autocast_mode.autocast(enabled=use_amp):
            ...
            specs = make_specs_from_signals(
                signals=batch["signals"], db_transform=db_transform, spec_module=spec_module
            )
...
def valid_one_epoch(
    ...
    for i in range(6):
        ...
        losses_per_class[f"valid/loss_{i}"] = loss_value.item()
...
def main() -> None:
    ...
    for fold in range(5):
        ...
        my_tools.train_one_fold(
            ...
            use_ema=True,
            pretrained_weights_fp=pathlib.Path(f"output/exp039/best_exp039_fold{fold}.pth"),
        )
        if run is not None:
            run.finish()
